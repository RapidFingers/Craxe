...
"wfenk6hLZT33+hGkUSUieS4HpKA6RJa0ZLnELfAWUAU=":
function (require, module, exports, __dirname, __filename) {
...
haxeshim_HaxelibCli.prototype = {
	...
	,path: function(libs) {
		...
		while(i < resolved.length) {
			...
			if(v == "--cwd") {
				...
				out.push(haxe_io_Path.addTrailingSlash(resolved[++i]));
	...
	,runDir: function(name,path,args) {
		tink_core__$Promise_Promise_$Impl_$.next(tink_core__$Promise_Promise_$Impl_$.next(haxeshim_Fs.get("" + path + "/haxelib.json",{ fileName : "haxeshim/HaxelibCli.hx", lineNumber : 61, className : "haxeshim.HaxelibCli", methodName : "runDir"}),function(s) {
			try {
				return new tink_core__$Future_SyncFuture(new tink_core__$Lazy_LazyConst(tink_core_Outcome.Success(JSON.parse(s).mainClass)));
			} catch( e ) {
				return new tink_core__$Future_SyncFuture(new tink_core__$Lazy_LazyConst(tink_core_Outcome.Failure(tink_core_TypedError.withData(null,"failed to parse haxelib.json",((e) instanceof js__$Boot_HaxeError) ? e.val : e,{ fileName : "haxeshim/HaxelibCli.hx", lineNumber : 66, className : "haxeshim.HaxelibCli", methodName : "runDir"}))));
			}
		}),function(mainClass) {
			if(mainClass == null) {
				var o = ["" + path + "/run.n"].concat(args).concat([haxe_io_Path.removeTrailingSlashes(process.cwd()) + "/"]);
				return new tink_core__$Future_SyncFuture(new tink_core__$Lazy_LazyConst(haxeshim_Exec.sync("neko",path,o,haxeshim__$Env_Env_$Impl_$.ofVars({ HAXELIB_RUN : "1", HAXELIB_LIBNAME : name}))));
			} else {
				var v = mainClass;
				return new tink_core__$Future_SyncFuture(new tink_core__$Lazy_LazyConst(tink_core_Outcome.Failure(new tink_core_TypedError(null,"mainClass support not implemented yet",{ fileName : "haxeshim/HaxelibCli.hx", lineNumber : 73, className : "haxeshim.HaxelibCli", methodName : "runDir"}))));
			}
