...
"jJrhNITHVtznRn8mdjLoITNszPr+Vx5lYeB0Ah4TYPU=":
function (require, module, exports, __dirname, __filename) {
...
var haxeshim_ServerKind = $hxEnums["haxeshim.ServerKind"] = { __ename__ : true, __constructs__ : ["Port","Stdio"]
	,Port: ($_=function(num) { return {_hx_index:0,num:num,__enum__:"haxeshim.ServerKind",toString:$estr}; },$_.__params__ = ["num"],$_)
	,Stdio: {_hx_index:1,__enum__:"haxeshim.ServerKind",toString:$estr}
};
var haxeshim_StdioState = $hxEnums["haxeshim.StdioState"] = { __ename__ : true, __constructs__ : ["AwaitingHeader","AwaitingData"]
	,AwaitingHeader: ($_=function(buf) { return {_hx_index:0,buf:buf,__enum__:"haxeshim.StdioState",toString:$estr}; },$_.__params__ = ["buf"],$_)
	,AwaitingData: ($_=function(buf,total) { return {_hx_index:1,buf:buf,total:total,__enum__:"haxeshim.StdioState",toString:$estr}; },$_.__params__ = ["buf","total"],$_)
};
...
haxeshim_CompilerServer.prototype = {
	...
	,stdio: function() {
		...
		var state = haxeshim_StdioState.AwaitingHeader(js_node_buffer_Buffer.alloc(0));
		var frame = function(payload) {
			var ret = js_node_buffer_Buffer.alloc(4 + payload.length);
			ret.writeInt32LE(payload.length,0);
			payload.copy(ret,4);
			return ret;
		};
		var processData = function(data) {
			var postfix = js_node_buffer_Buffer.alloc(0);
			...
			var first = js_node_buffer_Buffer.from(haxeshim_HaxeCli.checkClassPaths(_gthis.scope.resolve(ctx1.args)).join("\n"));
		...
		var reduce = function() {
			while(true) {
				...
				case 0:
					var buf = state.buf;
					next = buf.length >= 4 ? haxeshim_StdioState.AwaitingData(buf.slice(4),buf.readInt32LE(0)) : state;
					break;
				case 1:
	...
	,parseArgs: function(raw) {
		...
		if(_g == -1) {
			version = this.lastVersion == null ? this.scope.haxeInstallation.version : this.lastVersion;
		} else {
			var v = _g;
		...
		var server = js_node_Net.createServer(function(cnx) {
			...
			cnx.on("data",function(chunk) {
				...
				if(_g == -1) {
					...
					if(_g1 == -1) {
						version = _gthis.lastVersion == null ? _gthis.scope.haxeInstallation.version : _gthis.lastVersion;
					} else {
						var v1 = _g1;
						...
						return tink_core__$Future_Future_$Impl_$.async(function(cb1) {
							...
							connect = function(attempt) {
								...
								cnx.on("error",function(e) {
									if(attempt >= max) {
										var connect1 = tink_core_Outcome.Failure(new tink_core_TypedError(null,"Failed to connect to 127.0.0.1:" + port + " after " + max + " attempts because " + e,{ fileName : "haxeshim/CompilerServer.hx", lineNumber : 274, className : "haxeshim.CompilerServer", methodName : "connect"}));
										cb1(connect1);
									} else {
										var f3 = connect;
										var attempt1 = attempt + 1;
...
haxeshim_HaxeCli.checkClassPaths = function(args) {
	var i = 0;
	var invalid = [];
	while(i < args.length) if(args[i++] == "-cp") {
		var cp = args[i++];
		try {
	...
	,dispatch: function(args) {
		var getScope = function() {
			...
			return haxeshim_Exec.gracefully(function() {
				return haxeshim_Scope.seek(options);
			});
		};
		var _hx_tmp;
		var _hx_tmp1;
		...
		if(wait >= 0 && wait < args.length - 1) {
			...
			if(_hx_tmp1.length == 2) {
				if(_hx_tmp1[0] == "--run") {
					...
					if(haxeShimExtension.indexOf("-") != -1 && haxeShimExtension.toLowerCase() == haxeShimExtension) {
						...
						case "install-libs":
							...
							case 0:
								...
								break;
							case 1:
								tmp1 = args1[0] == "--silent" ? true : haxeshim_Exec.die(422,"unexpected arguments " + Std.string(args1));
								break;
							default:
						...
						var args3 = args;
						var scope1 = getScope();
						var _g2 = args3.indexOf("--haxe-version");
						if(args3.indexOf("--connect") == -1) {
							var tmp2 = _g2 == -1;
						} else if(_g2 == -1) {
							args3.push("--haxe-version");
							args3.push(scope1.haxeInstallation.version);
						}
					...
					var args5 = args;
					var scope2 = getScope();
					var _g3 = args5.indexOf("--haxe-version");
					if(args5.indexOf("--connect") == -1) {
						var tmp3 = _g3 == -1;
					} else if(_g3 == -1) {
						args5.push("--haxe-version");
						args5.push(scope2.haxeInstallation.version);
					}
				...
				var args7 = args;
				var scope3 = getScope();
				var _g4 = args7.indexOf("--haxe-version");
				if(args7.indexOf("--connect") == -1) {
					var tmp4 = _g4 == -1;
				} else if(_g4 == -1) {
					args7.push("--haxe-version");
					args7.push(scope3.haxeInstallation.version);
				}
...
haxeshim_Resolver.prototype = {
	...
	,process: function(args) {
		...
		while(_g < _g1) {
			...
			var tmp = haxeshim_Resolver.interpolate(args1[a],this.defaults);
			args1[a] = tmp;
		}
		var next = function() {
			if(i >= max) {
				_gthis.errors.push(new tink_core_TypedError(null,"missing argument for " + args1[i - 1],{ fileName : "haxeshim/Resolver.hx", lineNumber : 200, className : "haxeshim.Resolver", methodName : "process"}));
				return "<MISSING>";
			} else {
				i += 1;
				return args1[i - 1];
		...
		while(i < max) {
			i += 1;
			var _g2 = StringTools.trim(args1[i - 1]);
			switch(_g2) {
			case "":
				break;
			case "--cwd":
			...
			case "-lib":
				...
				case "scoped":
					if(lib[0].indexOf(":") == -1) {
						var _g21 = this.resolveInScope(lib[0]);
						if(_g21._hx_index == 1) {
							var e = _g21.failure;
							this.errors.push(e);
						}
...
lix_cli_HaxeCmd.ensure = function() {
	return tink_core__$Promise_Promise_$Impl_$.next(lix_cli_HaxeCmd.ensureScope(),function(_) {
		var scope = haxeshim_Scope.seek();
		var hx = new lix_client_haxe_Switcher(scope,false,Sys.println);
		var tmp;
		if(!sys_FileSystem.exists(hx.scope.haxeInstallation.path)) {
			...
			process.stdout.write("\n");
			tmp = hx.install(version,{ force : false});
		} else {
			tmp = new tink_core__$Future_SyncFuture(new tink_core__$Lazy_LazyConst(tink_core_Outcome.Success(tink_core_Noise.Noise)));
		}
		return tink_core__$Promise_Promise_$Impl_$.next(tmp,function(_1) {
			return new tink_core__$Future_SyncFuture(new tink_core__$Lazy_LazyConst(tink_core_Outcome.Success(tink_core_Noise.Noise)));
		});
	});
};
...
var lix_client_haxe_ResolvedUserVersionData = $hxEnums["lix.client.haxe.ResolvedUserVersionData"] = { __ename__ : true, __constructs__ : ["RNightly","ROfficial","RCustom"]
...
var lix_client_haxe_PickOfficial = $hxEnums["lix.client.haxe.PickOfficial"] = { __ename__ : true, __constructs__ : ["StableOnly","IncludePrereleases"]
...
var lix_client_haxe_UserVersionData = $hxEnums["lix.client.haxe.UserVersionData"] = { __ename__ : true, __constructs__ : ["UEdge","ULatest","UStable","UNightly","UOfficial","UCustom"]
